...
class AttachmentStorage:
    ...
    def save_bytes(
        ...
        """Save generated bytes as a session-owned attachment."""
        original_name = Path(name).name
        extension = Path(original_name).suffix.lower()
        if not original_name or extension not in ALLOWED_TYPES:
            raise AttachmentValidationError("生成文件的类型不受支持。")
        if not data:
            raise AttachmentValidationError("生成文件不能为空。")
        if len(data) > MAX_FILE_SIZE:
            raise AttachmentValidationError("生成文件不能超过 10 MB。")
        self._validate_content(extension, data)

        resolved_content_type = content_type or mimetypes.guess_type(original_name)[0]
        if resolved_content_type not in ALLOWED_TYPES[extension]:
            resolved_content_type = next(
                item for item in ALLOWED_TYPES[extension]
                if item not in {"application/zip", "application/octet-stream"}
            )
    ...
    def get_path(
        ...
        """Resolve a session-owned attachment to its on-disk path."""
        attachment = self.require_owned(session_id, [attachment_id])[0]
        path = self._session_dir(session_id, create=False) / (
            f"{attachment.id}{attachment.extension}"
        )
        if not path.is_file():
            raise AttachmentValidationError("附件文件不存在。")
